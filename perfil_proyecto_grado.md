...
* Herencia de Permisos por Rol Organizacional (RBAC): El motor de autorización del backend valida dinámicamente si un usuario posee acceso (lectura o escritura) sobre una rama del árbol DMS, cruzando la relación con el rol institucional asignado en el Catálogo de Personas. Esto simplifica drásticamente la administración de permisos en organizaciones complejas.
* Múltiples Vistas Guardadas del Usuario: Para optimizar la experiencia de usuario (UX) en la navegación de árboles masivos, la base de datos almacena el estado jerárquico de carpetas expandidas y colapsadas de forma personalizada. Cada operador puede guardar múltiples combinaciones de visualización lógicas y físicas independientes de los demás usuarios.
